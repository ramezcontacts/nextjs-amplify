...
class Helper {
  ...
  getSQLQryForSku(
    ...
    switch (productCategory) {
      ...
      LEFT JOIN product_prices AS GspPrice
        ON (GspPrice.product_id = ChildProduct.id AND GspPrice.price_type_id = 4
          AND GspPrice.is_deleted = 0 AND Manufacturer.is_gsp = 1)
      ...
      case "CONTACTLENS":
        skuQry = `SELECT DISTINCT 
        ChildProduct.id as sku_code, AppProduct.id as parent_id, AppProduct.product_classification_id as sku_cf_id, AppProduct.product_name as sku_name, AppProduct.has_valid_main_image_url as sku_image, AppProduct.short_desc as sku_desc, ListPrice.price AS sku_list_price, SellPrice.price AS sku_sell_price, GspPrice.price AS sku_gsp_price, COALESCE(ProductInventoryTotal.total_qty, 0) AS sku_stock, NULL AS sku_color, NULL AS sku_frame_size, NULL AS sku_power, (SELECT GROUP_CONCAT(CONCAT(product_shippable_setup_attribute_type_id, '::', value_varchar)) FROM product_shippable_to_attribute_type_values WHERE product_id = ChildProduct.id) AS sku_attributes, Manufacturer.default_markup_readers 
    ...
    //console.log(finalQry);process.exit();
    return finalQry;
  }
}
